...
from fb_video_url_get import get_linklist_vertica
from fb_video_fingerprint_export import get_image_export
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print('Usage: python main.py  output_file output_path date tmp_videopath')
        sys.exit(1)
    ...
    table_name = "datamining.dmn_fb_video_fingerprint"
    ...
    for video in video_list:
        ...
        pool.apply_async(downLoadVideo, (video, video_path, result_queue,))
        if count % 20 == 0:
            break
    pool.close()
