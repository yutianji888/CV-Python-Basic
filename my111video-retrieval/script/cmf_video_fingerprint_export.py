...
class get_image_export(object):
    ...
    def batch_insert_records(self, table_name, records):
        ...
        sql = """INSERT INTO {0} (cf_reference_detail, video_url, totalFrameNumber, frameRate, img_size, phash, insert_time) \
    ...
    def get_json_list(self,file_dir):
        ...
        fileList = [os.path.join(root, file) for root, dirs, files in os.walk(file_dir) for file in files if os.path.splitext(file)[1] == '.json' and re.findall('get_image_fingerprint.'+str(self.date), file)]
        return fileList
    ...
    def run(self):
        ...
        params_list = ["cf_reference_detail", "video_url", "totalFrameNumber", "frameRate", "img_size", "phash"]
